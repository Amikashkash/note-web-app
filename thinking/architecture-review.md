...
| AS-1 | יעד "בלי כפתורי שמירה": פתק חדש נשמר רק בלחיצה, ואין undo או היסטוריה (סעיף 11) | גבוה | היסטוריה מוקדם. מודל התוכן לפני MCP שלב 2 |
| SN-1 | תכונה חדשה: פתקים וקטגוריות "רגישים" שאינם נראים ל-MCP (סעיף 12) | גבוה | צד האפליקציה לפני MCP (C6). האכיפה ב-MCP שלב 1 |
...
- **תיקון:**
  ...
  - ~~זו שאלה פתוחה מס' 10 ב-`mcp-plan.md`, וההמלצה כאן היא **כן**.~~ **החלטה: מראות מתועדות עכשיו, בלי חבילה משותפת** (`mcp-plan.md` §9, שאלה 10). המינימום (טיפוסים ב-`src/types/template.ts`) כבר בוצע ב-A2.
...
- **תיקון** (פירוט ב-`mcp-plan.md` §7.3):
  ...
  > **עדכון לאור יעד "בלי כפתורי שמירה" (סעיף 11):** `runTransaction` בלקוח **לא עובד offline**, ולכן סעיפים 3 ו-4 הם פתרון ביניים בלבד. הפתרון הקבוע מתואר ב-§11.7:
  > - בתבניות רשימה: כתיבות ברמת שדה, כש-`items` הוא map במסמך.
  > - בטקסט חופשי: בדיקת `revision` ב-rules, שנאכפת גם על כתיבות שהמתינו offline.
...
- **תיקון:**
  ...
  - תיקון ההערה ב-`sw.ts`.
- **תזמון:** ~~יכול לחכות~~. **עודכן:** זה תנאי ליעד "בלי כפתורי שמירה" (§11.6). ה-persistence מופעל יחד עם מודל ה-conflicts החדש (§11.7) ולא לפניו, כי persistence בלי merge מגדיל את חלון הדריסה.
...
## 11. יעד מוצר: בלי כפתורי שמירה ("מה שנכנס נשאר")

**היעד:** יצירה ועריכה של פתקים נשמרות אוטומטית, כולל פתק חדש. כפתור מפורש נשאר רק במקום שבו המשתמש מקבל החלטה אמיתית.

### AS-1. המצב היום

- **חומרה:** גבוה (כפער מול יעד המוצר)
- **ראיות:**

| מקום | התנהגות היום | ראיה |
|---|---|---|
| פתק חדש | טופס מודאלי עם "שמור פתק". כותרת חובה. סגירה בלי לחיצה מאבדת הכל | `NoteForm.tsx:48-51,61-69,232-233` |
| עריכת פתק קיים | autosave ב-`NoteView`, אבל רק ל-`title` ו-`content` | `NoteView.tsx:85-99` |
| תגיות וצבע של פתק קיים | **אין דרך לערוך.** מצב העריכה של `NoteForm` הוא קוד מת: `editingNote` לעולם לא מקבל פתק, כי כל הקריאות הן `setEditingNote(null)` | `CategoryItem.tsx:45,93,171,227,267`, `CategoryView.tsx:37,106,155,202,246` |
| קטגוריה | "צור קטגוריה" / "שמור שינויים" | `CategoryForm.tsx:106-110` |
| שיתוף עם משתמש | "שתף" (החלטה, נשאר) | `ShareManagement.tsx:130-139` |
| דף Share | בחירת יעד ושמירה (החלטה, נשאר) | `Share.tsx:558` |
| שמירה בסגירת האפליקציה | `flush` רק ב-unmount. PWA שנהרגת ברקע מאבדת את מה שהמתין ב-debounce | `useDebouncedCallback.ts:69-75` |
| Undo / היסטוריה | אין בכלל. הארכיון הוא ה-"undo" היחיד, ורק לפתק שלם | – |

**למה זה חשוב מעבר ל-UX:** autosave מגדיל פי כמה את מספר הכתיבות הקטנות. בלי שינוי מודל, כל אחת מהן היא דריסה מלאה של `content` (C-1). כלומר היעד **מחמיר** את בעיות ה-concurrency אם מיישמים אותו לפני שהן נפתרות.

### 11.1 עקרונות

1. **כל קלט נשמר תוך שנייה, בלי פעולה.** המשתמש אף פעם לא צריך "לזכור לשמור".
2. **אין "יצירה" נפרדת מ"עריכה".** פתק חדש הוא פתק קיים שעוד לא נכתב ל-Firestore.
3. **שמירה אינה החלטה, ומחיקה כן.** פעולה הרסנית מקבלת undo (toast) או אישור. כתיבה רגילה לא מקבלת כלום.
4. **כל שינוי ניתן לשחזור:** undo מקומי בסשן, והיסטוריית גרסאות בשרת.
5. **offline הוא מצב רגיל ולא שגיאה.**
6. **כתיבות קטנות ותכופות חייבות להיות בטוחות להתנגשות** ברמת השדה או הפריט, ולא ברמת המסמך.

### 11.2 עורך אחד: `NoteEditor`

- `NoteForm` ו-`NoteView` מתאחדים לקומפוננטה אחת עם אותו pipeline שמירה לפתק חדש ולפתק קיים.
- תגיות וצבע נערכים inline וגם הם נשמרים אוטומטית. זה סוגר את הפער שבו אי אפשר לערוך אותם היום.
- `NoteForm` נמחק, יחד עם מצב העריכה המת שלו.
- `saveNote` ב-`useNoteEditor.ts:38-71` מוחלף ב-`useNoteDraft(noteId)` (ראו 11.4).

### 11.3 פתק חדש: יצירה עצלה עם מזהה מהלקוח

1. **"+"** פותח את העורך עם תבנית:
   - "+" רגיל פותח plain.
   - בחירת תבנית היא הקלט הראשון בעורך ולא שלב נפרד.
2. **המזהה נוצר בלקוח:** `doc(collection(db, 'notes')).id`. בשלב הזה **שום דבר לא נכתב**.
3. **כתיבה ראשונה = `setDoc`** של הפתק המלא, ברגע שיש תוכן משמעותי:
   - כותרת לא ריקה, או תוכן לא ריק, או פריט עם טקסט.
   - `setDoc` עם id קבוע הוא אידמפוטנטי. הוא עובד offline, והפתק מופיע מיד ברשימה מה-cache המקומי.
   - בניגוד ל-`addDoc` היום (`notes.ts:59`).
4. **כל הכתיבות הבאות הן `update`**, בתור טורי אחד לכל פתק (11.5), כך ש-update לא ייצא לפני ה-create.
5. **כותרת לא חובה:**
   - הדרישה ב-`NoteForm.tsx:48-51` מוסרת.
   - בתצוגה מוצגת כותרת נגזרת: השורה הראשונה או הפריט הראשון, או "ללא כותרת".
   - `LENGTH_LIMITS.NOTE_TITLE` נשאר.
6. **`order`:** מפתח fractional בסוף הקטגוריה (C-5), ולא `notes.length`.
7. **שיתוף:** בקטגוריה משותפת הפתק נוצר לפי החלטת SH-3.
8. **החלפת תבנית בפתק קיים** היא החלטה אמיתית: המרה מפורשת (A2), ואחריה toast של undo.

### 11.4 פתקים ריקים ונטושים

| מצב | טיפול |
|---|---|
| נפתח ונסגר בלי קלט | שום דבר לא נכתב (יצירה עצלה). אין מה לנקות |
| הוקלד תו אחד ונמחק, ואז נסגר | ב-`close`: אם `codec.isEmpty(note)` **וגם** המשתמש הבעלים **וגם** הפתק לא משותף, הפתק נמחק, ומוצג toast "פתק ריק הוסר · בטל". ה-"בטל" יוצר אותו מחדש מהעותק המקומי |
| ריק אבל משותף | נשאר. אולי מישהו אחר מוסיף לו עכשיו |
| האפליקציה נהרגה לפני ה-close | הפתק נשאר ריק ב-Firestore. **ניקוי בשרת:** פונקציה יומית (ראו למטה) |
| פתק ריק ברשימה | מוצג רגיל ("ללא כותרת") כדי שהמשתמש יוכל להמשיך בו. לא מסתירים |

- `isEmpty` הוא שדה boolean שנכתב **בכל** כתיבה, גם כ-`false`. זה אותו לקח של `reminderPending`: שאילתה לא מחזירה מסמכים שחסר בהם השדה.

**ניקוי בשרת (החלטה): `isEmpty` הוא רמז לשאילתה בלבד, ולעולם לא הבסיס להחלטה.**
- הפונקציה היומית שולפת מועמדים: `isEmpty == true` ו-`updatedAt` ישן מ-7 ימים.
- לכל מועמד היא **מחשבת מחדש** את הריקנות מהתוכן בפועל, דרך אותו codec (D-1/D-2): `title`, `content`/`items` (בלי tombstones), `tags`.
- הדגל נכתב על ידי הלקוח. לקוח עם באג, לקוח ישן, או שותף זדוני יכולים לכתוב `isEmpty: true` על פתק מלא. אם השרת היה סומך עליו, הוא היה מוחק תוכן.
- **מוחקת רק אם כל התנאים מתקיימים**, בתוך transaction שקורא את המסמך מחדש:
  - הפתק באמת ריק לפי ה-codec.
  - הוא של הבעלים שלו: הניקוי מתייחס לבעלים של הפתק ולא לכותב האחרון.
  - אין לו `sharedWith`, והקטגוריה שלו לא משותפת.
  - `updatedAt` לא השתנה מאז השאילתה.
- כשהחישוב לא מסכים עם הדגל, רושמים warning ומתקנים את `isEmpty` ל-`false`. כך מגלים לקוחות שכותבים דגל שגוי.
- מחיקת פתק ריק היא ניקוי מערכת ולא "hard delete" של תוכן. לכן היא לא סותרת את "אין מחיקה סופית" של MCP.

### 11.5 מחזור השמירה: `NoteSaveQueue`

- **תור לכל פתק**, שמחליף את `useDebouncedCallback` בעורך:
  - **צבירת patch** (C-4): `{...pending, ...next}`, ולכל פריט merge לפי `itemId`.
  - **debounce** של 800ms, עם **max-wait** של 5 שניות, כדי שהקלדה רצופה עדיין תישמר.
  - **flush** ב-close, בניווט, ב-`visibilitychange: hidden` וב-`pagehide`. זה חובה ב-PWA במובייל, שנהרגת ברקע בלי unmount.
  - כתיבות טוריות: create לפני update.
- **חיווי במקום כפתור**, שורת מצב קטנה בעורך:
  - "נשמר"
  - "שומר…"
  - "ממתין לחיבור": לפי `hasPendingWrites` ב-metadata של ה-snapshot.
  - "לא נשמר · נסה שוב": כתיבה נדחתה (ראו 11.6).
- **אין `await` על הכתיבה ב-UI.** ה-promise של Firestore מסתיים רק כשהשרת מאשר, ו-offline הוא לא מסתיים. המצב נגזר מה-cache המקומי.

### 11.6 Offline

1. **`persistentLocalCache` + `persistentMultipleTabManager`** (C-6 הופך לחובה). בלי זה, "מה שנכנס נשאר" לא נכון ברגע שסוגרים טאב offline.
2. **transactions בלקוח לא עובדים offline.** לכן כל פתרון concurrency שמבוסס על `runTransaction` בלקוח (C-1 סעיפים 3 ו-4, C-3, `mcp-plan.md` §7.3.3) הוא ביניים בלבד. המודל הקבוע (11.7) משתמש רק בכתיבות שדה ובבדיקות rules, ששתיהן עובדות offline.
   - בשרת, כלומר ב-MCP ובטריגר, transactions ממשיכים להיות בסדר.
3. **יומן מקומי לכתיבות שנדחו:**
   - Firestore שומר כתיבות ממתינות ב-IndexedDB. אבל כתיבה ש**נדחתה** בסנכרון (למשל התנגשות `revision`, 11.7) נזרקת, וה-cache חוזר לגרסת השרת. **הטקסט של המשתמש נעלם.**
   - `draftJournal` קטן ב-IndexedDB: לכל פתק, ה-patch האחרון שעוד לא אושר, יחד עם ה-`baseRevision`.
   - בדחייה, ה-journal מפעיל merge (11.7) במקום לאבד.
   - הרשומה נמחקת כשהשרת מאשר.
4. **פתק חדש offline** עובד מלא בזכות 11.3 (מזהה מהלקוח + `setDoc`).
5. **גרסת לקוח מינימלית:**
   - PWA שומרת גרסאות ישנות ב-cache.
   - לפני שינוי פורמט (11.7), מסמך `config/app` עם `minClientVersion`. לקוח ישן מציג "יש לרענן" ולא כותב.
   - בלי זה, לקוח ישן שלא מכיר את הפורמט החדש ידרוס אותו.

### 11.7 בטיחות התנגשויות לכתיבות קטנות ותכופות

הבעיה מ-C-1, מוכפלת ב-autosave: כל שינוי קטן כותב את **כל** מחרוזת ה-JSON. הפתרון: להפוך את יחידת הכתיבה ליחידת השינוי.

**א. תבניות רשימה (checklist, shopping, workplan, accounting): `items` כ-map בתוך מסמך הפתק**

```
notes/{id} {
  contentFormat: 2,
  items: {
    "k3f9": { text: "חלב", completed: false, pos: "a0", dueDate?, dueTime?, repeat?, ... },
    "k3fa": { text: "לחם", completed: true,  pos: "a1" },
    "k3fb": { deleted: true, deletedAt: <ts> }        // tombstone
  },
  revision, updatedBy, isEmpty, ...
}
```

- **כל שינוי הוא עדכון field-path:**
  - `updateDoc(ref, { 'items.k3f9.completed': true, revision: increment(1), updatedBy: uid })`.
  - שני אנשים שמסמנים פריטים שונים לא מתנגשים לעולם.
  - אותו פריט: last-write-wins **רק על השדה הזה**.
  - הכתיבה עובדת offline, בלי transaction.
- **הוספה** היא מפתח חדש (מזהה מהלקוח). **סדר** נקבע לפי `pos` (fractional index), כך שגרירה היא כתיבה אחת.
- **מחיקה היא tombstone** ולא `deleteField()`. אחרת כותב ישן שמעדכן `items.x.completed` "מחייה" פריט חלקי. ה-tombstone גם מאפשר undo. הטריגר מנקה tombstones אחרי 30 יום.
- **למה map ולא subcollection** (שנדחה ב-`mcp-plan.md` §7): הכל נשאר במסמך אחד. הטריגר, הגיבוי ו-MCP קוראים מסמך אחד, והמאזין היחיד (ST-3) לא משתנה.
- **מגבלות:** מסמך עד 1MB (אלפי פריטים). חובה `fieldOverrides` שמחריג את `items` מאינדוקס, אחרת כל פריט מייצר index entries ועלות כתיבה, ויש תקרה של 40K entries.
- **מיגרציה:**
  - ה-codec (D-1) קורא `items` אם `contentFormat == 2`, אחרת את `content` הישן.
  - בעריכה הראשונה הוא כותב `items` + `contentFormat: 2` + `content: ''`.
  - הטריגר, `backupFormat` ו-MCP קוראים דרך אותו codec (D-2), וזה מה שמאפשר את המיגרציה.
  - חובה לפני כן: `minClientVersion` (11.6.5).
- **מתכון:** `servings`, `prepTime` ו-`cookTime` כשדות. `ingredients` ו-`instructions` כשני maps באותו מבנה.

**ב. טקסט חופשי (plain): בדיקת `revision` ב-rules**

- אין merge ברמת שדה למחרוזת. לכן: optimistic concurrency שנאכפת **בשרת בזמן הסנכרון**, ולכן עובדת גם offline.
  - כתיבת טקסט שולחת `revision: base + 1` כערך מפורש.
  - rule ב-update: `request.resource.data.revision == resource.data.revision + 1`.
  - כתיבת פריט (סעיף א') שולחת `increment(1)`. ה-rule רואה את הערך אחרי ה-transform, ולכן היא תמיד עוברת.
  - כתיבות טוריות מאותו מכשיר (rev 6, ואז 7) מוחלות בסדר, ולכן עוברות.
- **בדחייה** (מכשיר אחר כתב בינתיים), **החלטה: "שמור את שתי הגרסאות" בלבד**:
  - ה-journal (11.6.3) כותב מחדש את הגרסה של השרת, ומוסיף בסופה את הגרסה המקומית כבלוק "⚠ עריכה מקבילה ממכשיר אחר", עם `revision` חדש.
  - מוצגת הודעה שמבקשת מהמשתמש לאחד בעצמו.
  - **אף פעם לא זורקים טקסט.**
  - **אין merge אוטומטי** (`diff-match-patch` או דומה) בשלב הזה. מוסיפים אותו רק אם יתגלו התנגשויות אמיתיות. לשם כך כל התנגשות כזו נרשמת (אירוע אנונימי, בלי תוכן), כדי שתהיה החלטה מבוססת מדידה.
- **`title`:** שדה קצר, last-write-wins. `tags` עם `arrayUnion`/`arrayRemove`. `color` ו-`isPinned` הם שדות בודדים.

**ג. מה זה פותר מהסקירה**

- **C-1:** אין יותר דריסת מסמך. `NoteView` מציג תמיד את ה-snapshot החי, והטיוטה המקומית היא רק patch ממתין.
- **C-4:** צבירת patch.
- **C-5:** `pos` fractional במקום reorder של N כתיבות.
- **F-2:** הטריגר עושה reconcile מהמסמך.
- **`mcp-plan.md` §5 ו-§7:** `add_checklist_item` הופך לכתיבת שדה אחת, בלי merge. התכנית עודכנה למודל הזה.

**ד. כתיבות שרת (MCP ופונקציות) עוקפות את ה-rules**

- ה-Admin SDK לא עובר דרך `firestore.rules`, ולכן ה-rule של `revision == old + 1` **לא מגן** מפני כתיבות של שרת ה-MCP או של הטריגרים.
- לכן כל כתיבת טקסט חופשי מהשרת נעשית ב-**transaction בצד השרת**, שאוכף בדיוק את אותו כלל:
  - קורא את המסמך.
  - משווה את `revision` ל-`expectedRevision` שה-tool קיבל מ-`get_note`.
  - כשיש התאמה: כותב את הטקסט עם `revision + 1` ו-`updatedBy: 'mcp:<clientId>'`.
  - כשאין התאמה: `Conflict`, ולא כותב.
- כתיבות פריט מהשרת (`items.<id>`) משתמשות ב-`increment(1)`, כמו הלקוח, וגם הן בתוך ה-transaction של ה-tool (נדרש ממילא בשביל ה-audit log).
- ראו `mcp-plan.md` §7.2. שם זה מופיע כחובה, והמקום האחד שאוכף זאת הוא `UserScope` / שכבת הכתיבה של `notesCore`, לא כל tool בנפרד.

**שדה חדש, `updatedBy`**, בכל כתיבה. ה-rule אוכף `== request.auth.uid`. MCP כותב `mcp:<clientId>` דרך Admin SDK. הוא משמש להיסטוריה (11.8) ולהצגת "עודכן ע"י".

### 11.8 Undo והיסטוריית גרסאות

שלוש שכבות, מהמיידי לארוך-טווח:

**שכבה 1: undo מקומי בסשן העריכה**
- מחסנית פעולות הפוכות בעורך: כפתור ↶, ובנוסף Ctrl/Cmd+Z מחוץ לשדות טקסט. בתוך שדה טקסט, ה-undo הטבעי של הדפדפן נשאר.
- undo הוא כתיבה רגילה דרך `NoteSaveQueue`. אין מצב מיוחד.

**שכבה 2: toast "בטל" לפעולות הרסניות בודדות** (6 עד 8 שניות)
- מחיקת פריט (tombstone ⇒ ביטול = `deleted: false`).
- ארכוב פתק.
- הסרת פתק ריק (11.4).
- המרת תבנית. **ה-undo חייב להחזיר את התוכן המקורי במדויק**, ולא לבנות אותו מחדש מהתוכן המומר (ראו ההערה בקבוצה G).
- העברת קטגוריה.
- "נקה מסומנים" ברשימת קניות.
- כך ה-`confirm()` הקיימים (`NoteView.tsx:103-106` ועוד) הופכים ל-undo, בלי חסימה.

**שכבה 3: היסטוריית גרסאות בשרת**
- **`notes/{noteId}/versions/{versionId}`**, נכתב **על ידי Cloud Function** על כתיבת פתק.
  - כך נתפסים כל הכותבים: כל המכשירים, שותפים, MCP ותיקונים עתידיים.
  - הלקוח לא כותב גרסאות, וכך אין כתיבה כפולה לכל autosave.
- **Coalescing:** גרסה חדשה נשמרת כש:
  - הגרסה האחרונה ישנה מ-10 דקות, **או**
  - `updatedBy` שונה מזה של הגרסה האחרונה, **או**
  - קרה אירוע מבני: ארכוב, המרת תבנית, העברת קטגוריה, מיגרציית `contentFormat`.
  - הגרסה שומרת את מצב ה-`before`, כך שתמיד אפשר לחזור למה שהיה לפני רצף עריכה.
- **תוכן הגרסה:** `title`, `content`/`items`, `templateType`, `tags`, `color`, `categoryId`, `updatedBy`, `at`, `reason`.
- **שמירה (retention):** 50 האחרונות לפתק + TTL של 90 יום (`expiresAt`). שאלה פתוחה.
- **rules:**
  - `read` למי שיכול לקרוא את הפתק: `get(/notes/$(noteId))` + הבדיקה הקיימת.
  - `write: if false`.
- **UI:**
...
- **קשר ל-audit log של MCP (`mcp-plan.md` §6):** רשומת audit מפנה ל-`versionId`, ולכן לא צריכה לשמור `before` מלא. לשקול איחוד.
- **חשוב: להקדים את שכבה 3.** היא לא תלויה בעורך החדש. היא רשת ביטחון גם לבאגים הקיימים (C-1 עד C-3) וגם למיגרציה המסוכנת של 11.7. לכן היא מופיעה כבר בקבוצה B בסדר התיקונים.

### 11.9 איפה נשארות פעולות מפורשות

| פעולה | למה החלטה |
|---|---|
| דף Share: בחירת קטגוריה ופתק יעד, ו"שמור" | בחירת יעד לתוכן חיצוני |
| `ShareManagement`: הוספת משתמש לשיתוף | חשיפת מידע לאדם אחר |
| שחזור גרסה | מחליף את המצב הנוכחי |
| המרת תבנית | משנה מבנה, עם undo |
| מחיקת קטגוריה, מחיקה סופית מהארכיון | בלתי הפיך |
| Login | – |
| **לא נשאר:** "שמור פתק", "עדכן פתק", "צור קטגוריה", "שמור שינויים" בקטגוריה | קטגוריה נוצרת inline: שם + Enter או blur. שם וצבע נשמרים אוטומטית |

### 11.10 בדיקות נדרשות (בנוסף ל-§8)

- **Unit:**
  - `NoteSaveQueue`: צבירה, max-wait, flush ב-`pagehide`, create לפני update.
  - codec v2: הוספה, tombstone, `pos`, `isEmpty`, מיגרציה מ-`content`.
  - 3-way merge של טקסט.
- **Rules:**
  - `revision` מפורש ישן נדחה, ו-`increment` עובר.
  - `updatedBy` זר נדחה.
  - `versions` לא ניתן לכתיבה מהלקוח.
- **Emulator:**
  - שני לקוחות מסמנים פריטים שונים במקביל, ושני הסימונים נשמרים.
  - לקוח offline כותב טקסט, השני כותב online, הראשון מתחבר, ואין אובדן.
  - גרסאות נוצרות לפי כללי ה-coalescing.
- **ידני:**
  - פתק חדש, הקלדה, ומצב טיסה. סגירת האפליקציה ופתיחה מחדש: הפתק שם.
  - פתק ריק שנסגר: נעלם עם "בטל".

### 11.11 החלטות (היו שאלות פתוחות)

| # | שאלה | החלטה |
|---|---|---|
| 1 | כותרת לא חובה? | **כן.** כותרת נגזרת בתצוגה (11.3) |
| 2 | פתק ריק: מחיקה או ארכוב? | **מחיקה בלקוח עם toast "בטל"**, וניקוי בשרת שמחשב ריקנות מחדש מהתוכן ולא סומך על `isEmpty` (11.4) |
| 3 | שמירת גרסאות | **50 גרסאות או 90 יום**, לפי מה שמגיע קודם |
| 4 | מיזוג טקסט בהתנגשות | **שמירת שתי הגרסאות בלבד** (בלוק התנגשות + הודעה). merge אוטומטי רק אם יתגלו התנגשויות אמיתיות (11.7ב) |
| 5 | `minClientVersion` | **כן** (11.6.5) |

---

## 12. תכונה חדשה: פתקים רגישים (SN-1)

**המטרה:** המשתמש יכול לסמן פתק או קטגוריה כ"רגישים", ואז Claude (דרך MCP) לא רואה אותם בכלל. ההגדרה המלאה בצד השרת נמצאת ב-`mcp-plan.md` §3.4.

### 12.1 מודל נתונים

- `notes.isSensitive: boolean` ו-`categories.isSensitive: boolean`.
- **רגישות אפקטיבית** של פתק = `note.isSensitive || category(note.categoryId).isSensitive`.
  - הרגישות **נגזרת בזמן קריאה ולא מועתקת לפתקים**. לכן פתק שנוסף לקטגוריה רגישה מאוחר יותר הוא רגיש אוטומטית, ואין סנכרון שעלול להתפספס (בניגוד למודל השיתוף, SH-3).
  - פתק שהבעלים מעביר מקטגוריה רגישה לרגילה מפסיק להיות רגיש, אלא אם יש לו דגל משלו. זו החלטת הבעלים, וה-UI מזהיר ("הפתק יהיה גלוי ל-Claude").
- השדה נכתב תמיד, כולל `false`, ביצירה ובעדכון. מסמך ישן בלי השדה נחשב לא רגיש.

### 12.2 הרשאות: רק הבעלים, ורק מהאפליקציה

- **rules**, ב-update של `notes` וגם של `categories`:
  - שינוי `isSensitive` (`request.resource.data.get('isSensitive', false) != resource.data.get('isSensitive', false)`) מותר **רק ל-`isOwner`**.
  - שותף לא יכול לסמן ולא לבטל.
- **חור שחייב להיסגר יחד:** שותף שמעביר פתק מקטגוריה רגישה לקטגוריה שלו מבטל את הרגישות האפקטיבית בפועל. לכן C6 כולל את חלק ה-rules של R-2: **שותף לא משנה `categoryId`**.
- **MCP:** אין שום tool שמשנה את הדגל. ה-allowlist של השדות ב-`sanitizeNotePatch` לא כולל אותו (ממילא אין לו גישה לפתקים רגישים).
- **פתק משותף:** הדגל של הבעלים חל על ה-Claude של **כל** המשתמשים שהפתק משותף איתם.
  - זה עובד כי `UserScope` רץ עם Admin SDK וקורא את הקטגוריה של הבעלים גם כשהיא לא משותפת עם המשתמש.

### 12.3 UI

- מתג "רגיש (מוסתר מ-Claude)":
  - בפתק (לבעלים בלבד).
  - בהגדרות הקטגוריה, כלומר `CategoryForm` או העריכה ה-inline של G7 (לבעלים בלבד).
- אייקון מנעול על פתקים וקטגוריות רגישים: `NoteCard`, `CategoryItem`, וכותרת הפתק ב-`NoteView`.
- **מגבלת תצוגה:** שותף שלא רואה את הקטגוריה של הבעלים (שיתוף פתק בודד) לא יכול לדעת שהיא רגישה. לכן המנעול לא יוצג אצלו, אף שהאכיפה ב-MCP כן חלה. זו מגבלה של התצוגה בלבד. אפשר לסגור אותה בהמשך בשדה `effectiveSensitive` שמתוחזק בטריגר, אם יתברר שצריך.
- הרגישות לא משנה שום דבר אחר באפליקציה: חיפוש, גיבוי, שיתוף ל-WhatsApp ותזכורות עובדים כרגיל. היא נוגעת רק לגישה של MCP.

### 12.4 מה הדגל לא עושה

- **לא מוחק מידע שכבר נקרא.** פתק שסומן רגיש אחרי ש-Claude קרא אותו בשיחה, עדיין נמצא בשיחה ההיא. ה-UI אומר זאת ליד המתג.
- **לא הצפנה.** הנתונים ב-Firestore ובגיבויים זהים. זו בקרת גישה של שרת ה-MCP בלבד.

### 12.5 בדיקות

- **rules:**
  - שותף לא יכול לשנות `isSensitive` של פתק ושל קטגוריה.
  - שותף לא יכול לשנות `categoryId`.
  - הבעלים יכול.
- **MCP:** ראו `mcp-plan.md` §3.4 (הבדיקה הגנרית רצה גם מול פתק רגיש).
...
| צעד | תוכן | בדיקה |
|---|---|---|
| A1 ✅ | C-4: ה-debounce צובר patch. **בוצע**, v1.20.1 | ידני: שינוי כותרת ואז V מהיר, ובדיקה ב-Firestore console ששניהם נשמרו |
| A2 ✅ | C-2: התבניות לא "מאתחלות" תוכן שלא פוענח. הודעה + "המר לטקסט". האתחול רק על `''`. **בוצע**, v1.20.2 | פתק plain ששונה ל-checklist → עריכה → הטקסט נשאר |
| A3 ✅ | C-3: Share "הוסף" רק ל-plain ו-workplan, או הוספה כפריט דרך פונקציית append. **בוצע**, v1.20.3 | שיתוף לרשימת קניות → נוסף מוצר, ה-JSON תקין |
| ~~A4~~ | **מדולג (החלטה):** `NoteForm` מוחלף ב-G2/G3, ולכן לא מתקנים אותו בנפרד. E-3 נפתר כשהטופס נעלם | – |

בנוסף, מחוץ לרשימה המקורית, בוצעו על אותו branch: S-2 (ביניים, v1.20.4), הוצאת `.claude/settings.local.json` מה-repo, ותיקון `useModalHistory` תחת StrictMode (v1.20.5). הכל ב-`main` מ-`edf475e`.
...
| C6 | §12 (SN-1), **צד האפליקציה בלבד**: שדה `isSensitive` בפתק ובקטגוריה (טיפוסים, mapper, כתיבה תמיד). rules: רק הבעלים משנה את הדגל, ושותף לא משנה `categoryId` (החלק הזה של R-2 מוקדם לכאן). מתגים ומנעולים ב-UI. האכיפה ב-MCP שייכת ל-`mcp-plan.md` | בדיקות rules (§12.5). ידני: שותף לא רואה את המתג, ובקונסול ניסיון לשנות נדחה |
...
| צעד | תוכן | בדיקה |
|---|---|---|
| D1 | D-2 (**הוחלט: מראות מתועדות, בלי חבילה משותפת**): מראה של `templateContent`/codecs ב-`functions/`, עם הערת הפניה בשני הצדדים. הטיפוסים כבר ב-`src/types/template.ts` (A2) | שתי החבילות מתקמפלות. בדיקה שמשווה את פלט המראה לפלט המקור על אותן דוגמאות |
...
| D3 | הוספת `revision`, `updatedBy` ו-`isEmpty` בכל כתיבה בלקוח | קיימים במסמכים אחרי עריכה |
| D4 | §11.7: החלטה סופית על `contentFormat: 2` (items כ-map). codec שקורא את שני הפורמטים. `fieldOverrides` שמחריג את `items` מאינדוקס. `config/app.minClientVersion` + חסימת כתיבה בלקוח ישן (§11.6.5) | unit: codec קורא v1 ו-v2. לקוח עם גרסה נמוכה מציג "יש לרענן" |

### קבוצה E: concurrency ושיתוף (לפני MCP שלב 2)

| צעד | תוכן | בדיקה |
|---|---|---|
| E1 | C-1 לתבניות רשימה (§11.7א): כתיבות field-path ל-`items.<id>`, tombstones, `pos`, ומיגרציה עצלה ל-`contentFormat: 2`. **מחליף** את גישת ה-transaction, שלא עובדת offline | שני דפדפנים מסמנים פריטים שונים במקביל → שניהם נשמרים. גם כשאחד offline |
| E2 | C-1 לטקסט חופשי (§11.7ב): rule של `revision == old + 1`, `draftJournal` ב-IndexedDB ו-3-way merge בדחייה. `NoteView` מציג תמיד את ה-snapshot החי | מכשיר offline עורך, השני online, חיבור מחדש → שום טקסט לא אבד |
| E2b | C-6 + §11.6: `persistentLocalCache` (multi-tab) + חיווי "ממתין לחיבור". **רק אחרי E1 ו-E2** | מצב טיסה → עריכה → סגירת האפליקציה → פתיחה → התוכן שם ומסתנכרן |
...
**כאן אפשר להתחיל את MCP שלב 2 (כתיבה).** לפני כן צריך לעדכן את `mcp-plan.md` §5 ו-§7 למודל `items`.

### קבוצה G: בלי כפתורי שמירה (§11). אחרי E, במקביל ל-MCP שלב 2

| צעד | תוכן | בדיקה |
|---|---|---|
| G1 | §11.5: `NoteSaveQueue`: צבירת patch, debounce 800ms עם max-wait של 5 שניות, flush ב-`pagehide`/`visibilitychange`, ושורת מצב ("נשמר" / "שומר…" / "ממתין לחיבור" / "לא נשמר") | unit + ידני: הקלדה ומעבר מיידי לאפליקציה אחרת במובייל → נשמר |
| G2 | §11.2: `NoteEditor` מאוחד לפתק קיים. תגיות וצבע inline עם autosave. מחיקת מצב העריכה המת של `NoteForm` | עריכת תגית בפתק קיים → נשמרת בלי כפתור |
| G3 | §11.3: פתק חדש: מזהה מהלקוח, יצירה עצלה ב-`setDoc`, כותרת לא חובה, כותרת נגזרת. מחיקת `NoteForm` | "+" → הקלדה → סגירה → הפתק ברשימה. "+" → סגירה מיד → שום דבר לא נוצר. אותו דבר offline |
| G4 | §11.4: פתק ריק בסגירה → מחיקה + "בטל". ניקוי יומי בשרת לפי `isEmpty` | הקלדה ומחיקה → סגירה → toast. "בטל" מחזיר |
| G5 | §11.8 שכבות 1 ו-2: undo מקומי בעורך + toast "בטל" לפעולות הרסניות, במקום `confirm()` | מחיקת פריט → "בטל" → חוזר. המרת checklist עם תאריכים לטקסט → "בטל" → התאריכים, הסימונים והתזכורות חוזרים |
| G6 | §11.8 שכבה 3 (UI): "היסטוריה" + שחזור | שחזור גרסה → התוכן חוזר. שחזור יוצר גרסה בעצמו |
| G7 | §11.9: קטגוריה inline (שם + Enter/blur), בלי "צור" או "שמור" | יצירה ושינוי שם בלי כפתור |

**הערה ל-G: המרת תבנית מאבדת מידע היום.**
- `convertContent` (`src/utils/templateContent.ts`, מ-A2) עובר דרך שורות טקסט, ולכן מאבד `dueDate`, `dueTime`, `repeat` ו-`completed`.
- כתוצאה, הטריגר מוחק את התזכורות של המשימות.
- היום זה קורה רק בטופס של פתק חדש, שבו אין עדיין תזכורות. אבל ב-G ההמרה עוברת לפתק קיים.
- **לכן ה-toast "בטל" של המרת תבנית (G5) חייב לשמור את התוכן המקורי המלא** (המחרוזת או ה-`items` לפני ההמרה) **ולכתוב אותו חזרה כמו שהוא**. כך הטריגר יוצר מחדש את התזכורות.
- תזכורת חד-פעמית שמועדה עבר בינתיים לא תחזור. זו ההתנהגות הרגילה של הטריגר.
- אם זה מתברר כלא מספיק: המרה ישירה בין checklist ל-shopping שמשמרת `completed` ↔ `checked`, והודעה לפני המרה שמוחקת תאריכים.
