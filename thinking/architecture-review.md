...
| האפליקציה נהרגה לפני ה-close | הפתק נשאר ריק ב-Firestore. **ניקוי בשרת:** פונקציה יומית מוחקת פתקים שבהם `isEmpty == true` ו-`updatedAt` ישן מ-7 ימים, רק לבעלים ורק כשאין `sharedWith` |
...
- **בדחייה** (מכשיר אחר כתב בינתיים): ה-journal (11.6.3) מבצע 3-way merge של טקסט (base, local, remote), למשל עם `diff-match-patch`.
  - אם ה-merge נקי, כותבים אותו.
  - אם לא, שומרים את שתי הגרסאות: הגרסה המקומית נוספת כבלוק "⚠ עריכה מקבילה ממכשיר אחר" בסוף הטקסט, ומוצגת הודעה.
...
- **`mcp-plan.md` §7:** `add_checklist_item` הופך לכתיבת שדה אחת, בלי merge. צריך לעדכן את התכנית אחרי ההחלטה.
...
### 11.11 שאלות פתוחות לעיצוב

1. כותרת לא חובה: מקובל?
2. פתק ריק: מחיקה (מוצע) או ארכוב?
3. שמירת גרסאות: 50 גרסאות / 90 יום?
4. מיזוג טקסט בהתנגשות: `diff-match-patch` אוטומטי (מוצע), או תמיד שתי גרסאות?
5. `minClientVersion`: מקובל לחסום כתיבה מלקוח ישן עד רענון?
...
| A4 | E-3: `NoteForm` נסגר רק בהצלחה. **ביניים בלבד:** `NoteForm` מוחלף ב-G2/G3. אפשר לדלג אם G מגיע בקרוב | אופליין או rules חוסמים → הטופס נשאר פתוח עם התוכן |
...
| G5 | §11.8 שכבות 1 ו-2: undo מקומי בעורך + toast "בטל" לפעולות הרסניות, במקום `confirm()` | מחיקת פריט → "בטל" → חוזר |
